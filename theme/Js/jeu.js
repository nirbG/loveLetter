        ...
        start : function () {
                ...
                pr.done(function (d, s, jqXHR) {
                    ...
                    if (d.status != "terminer") {
                        var cssmain = 1;
                        var cssscore = 1 + " col-sm-offset-4";
                        var cssplat = 1;
                        var cssplatadv = 12;
                        if (d.joueur.length - 1 == 2) {
                        ...
                        for (var z = 1; z < d.joueur.length; z++) {
                            var dom;
                            if (d.joueur.length - 1 == 1) {
                                dom = "#partie";
                            } else {
                                var plateauadv = "<div id='plateauadv" + z + "' class=' col-sm-" + cssplatadv + " '></div>";
                            ...
                            var score = "<div class=" + "'col-sm-" + cssscore + "' style='text-align: right;'><h3>Score:" + point + "</h3><p>" + user.pseudo + "</p></div>";
                            var cont = "";
                            var carte = "";
                            ...
                            if (jeux.nbCarte == 2) {
                                for (var i = 0; i < jeux.main.length; i++) {
                                    carte += "<div class='col-sm-" + cssmain + " '><img src='theme/ressource/cartes/Pioche.png'></div>";
                                }
                                cont += "<div id='mainAdv' class='row'>" + score + " " + carte + "</div>";
                            } else {
                                carte += "<div class='col-sm-" + cssmain + " '><img src='theme/ressource/cartes/" + carteAdv + "'></div>";
                            ...
                            if (jeux.size != 0) {
                                for (i = 0; i < jeux.size; i++) {
                                    cont += "<div class='col-sm-" + cssplat + " '><img data-img='theme/ressource/cartes/" + jeux.plateau[i].image + "' src='theme/ressource/cartes/" + jeux.plateau[i].image + "'> </div>";
                                }
                            } else {
                                cont += "<div class='col-sm-" + cssplat + " center '><img src='theme/ressource/cartes/vide.png'></div>";
                            }
                            cont += "</div>";
                            $(dom).append(cont);
                        }
                        ...
                        var tour = "<div  class='col-sm-1 col-sm-offset-3'><h5>C'est au tour de <strong>" + joue + "</strong></h5></div>";
                        for (i = 0; i < d.defausse.length; i++) {
                            def += "<div class='col-sm-1 carte '><img data-img='theme/ressource/cartes/" + d.defausse[i].image + "' src='theme/ressource/cartes/" + d.defausse[i].image + "'></div>";
                        }
                        if (nbC == 0) {
                            pio += "<div id='pioche' class='row'>" + tour + "<div id='pioche' class='col-sm-1'><img src='theme/ressource/cartes/vide.png'></div>" + def + "</div>";
                        } else {
                            pio += "<div id='pioche' class='row'>" + tour + "<div id='pioche' class='col-sm-1'><img src='theme/ressource/cartes/Pioche.png'></div>" + def + "</div>";
                        }
                        jeux = d.joueur[0];
                        $("#partie").append(pio);
                        ...
                        $("#partie").append(me);
                        var desc = "";
                        for (var i = 0; i < d.historique.length; i++) {
                            desc += d.historique[i].desc;
                        }
                        var historique = "<div class='historique'>" + desc + "</div>"
                        $("#partie").append(historique);
...
load.module.guard=(function() {
    return {effect: function (data) {
        ...
        for(var i=1;i<data.joueur.length;i++){
            ...
            if(data.joueur[i].action!="éliminé" && data.joueur[i].action!="protege") {
                Luser += "<div class='user col-sm-6' data='" + u.id + "'><img src='theme/ressource/user.png'><h4>" + u.pseudo + "</h4></div>";
            }
        }
        var user = "<div id='users' class='row'>";
        if(Luser=="") {
            user += "<div id='stopeffect' class=' col-sm-6'><img src='theme/ressource/quitter.png'><h4>Ne pas appliquer l'effet</h4></div>";
        }else{
            user +=Luser;
        }
        user+="</div>";
...
load.module.priest=(function() {
    return {
        effect: function (data) {
            ...
            var $blackout = $("<div class='blackout'>").css("display", "none");
            var back="<div style='	background-color: black; position:absolute; z-index:-1; top:0; left:0; right:0; bottom:0; opacity:0.5;'></div>";
            var h4 = $("<h1>").text("voir la main de quel utilisateur").css("margin", "0");
            var Luser = "";
            for(var i=1;i<data.joueur.length;i++){
                var u=data.joueur[i].user;
                if(data.joueur[i].action!="éliminé" && data.joueur[i].action!="protege") {
                    Luser += "<div class='user col-sm-6' data='" + i + "'><img src='theme/ressource/user.png'><h4>" + u.pseudo + "</h4></div>";
                }
            }
            var user = "<div id='users' class='row'>";
            if(Luser=="") {
            ...
            $(".blackout .user").click(function () {
                ...
                $thumbnail = $(this);
                var $blackout = $("<div class='blackout'>").css("display", "none");
                var user = "<div id='prince' class='row'>";
                var j1=data.joueur[idus];
                var j=data.joueur[0];
                var c = data.carte[i];
                var back = "<div style='	background-color: black; position:absolute; z-index:-1; top:0; left:0; right:0; bottom:0; opacity:0.5;'></div>";
                user += "<div class='crte col-lg-offset-3 col-sm-6' '><img src='theme/ressource/cartes/"+j1.main[0].image+"'></div>";
                user += "</div>";
                $blackout.append(back);
                $blackout.append(h4);
                $blackout.append(user);
...
load.module.baron=(function() {
    return {effect: function (data) {
        ...
        $(".blackout .user").click(function () {
            ...
            var j1=data.joueur[$(this).attr('data')];
            var j=data.joueur[0];
            var c = data.carte[i];
            var back = "<div style='	background-color: black; position:absolute; z-index:-1; top:0; left:0; right:0; bottom:0; opacity:0.5;'></div>";
...
load.module.handmaid=(function() {
    return {
        effect: function (data) {
            ...
            var $blackout = $("<div class='blackout'>").css("display", "none");
            var back="<div style='	background-color: black; position:absolute; z-index:-1; top:0; left:0; right:0; bottom:0; opacity:0.5;'></div>";
            var h4 = $("<h1>").text("Tu es protégé pour un tour").css("margin", "0");
            var user = "<div id='users' class='row'>";
            user += "<div id='stopeffect' class=' col-sm-12'><img src='theme/ressource/quitter.png'></div>";
            user+="</div>";
            $blackout.append(back);
            $blackout.append(h4);
            $blackout.append(user);
...
load.module.king=(function() {
    return {
        effect: function (data) {
            ...
            var $blackout = $("<div class='blackout'>").css("display", "none");
            var back="<div style='	background-color: black; position:absolute; z-index:-1; top:0; left:0; right:0; bottom:0; opacity:0.5;'></div>";
            var h4 = $("<h1>").text("click sur un utilisateur").css("margin", "0");
            var Luser = "";
            for(var i=1;i<data.joueur.length;i++){
                var u=data.joueur[i].user;
                if((data.joueur[i].action!="éliminé")&&(data.joueur[i].action!="protege")) {
...
load.module.princess=(function() {
    return {
        effect: function (data) {
            ...
            var $blackout = $("<div class='blackout'>").css("display", "none");
            var back="<div style='	background-color: black; position:absolute; z-index:-1; top:0; left:0; right:0; bottom:0; opacity:0.5;'></div>";
            var h4 = $("<h1>").text("Tu es éliminé").css("margin", "0");
            var user = "<div id='users' class='row'>";
            user += "<div id='stopeffect' class=' col-sm-12'><img src='theme/ressource/quitter.png'></div>";
            user+="</div>";
            $blackout.append(back);
